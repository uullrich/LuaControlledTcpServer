...
void TcpServer::start() {
    m_running = true;
    doAccept();
    std::cout << "TCP Server started on port " << m_acceptor.local_endpoint().port() << std::endl;
}
...
bool TcpServer::sendMessage(const std::string& sessionId, const CanMessage& message) {
...
TcpServer::Session::Session(tcp::socket socket, TcpServer& server, std::string id)
...
bool TcpServer::Session::send(const CanMessage& message) {
    try {
        ...
        std::cerr << "Error sending to client " << m_id << ": " << e.what() << std::endl;
        return false;
    }
}

std::string TcpServer::Session::getId() const {
    return m_id;
}
...
void TcpServer::Session::processMessage() {
    ...
    std::vector<uint8_t> canData(m_messageBuffer.begin() + 4, m_messageBuffer.end());
